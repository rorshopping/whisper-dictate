...
- **Voice commands (deterministic Command Mode subset)** — new `voice_commands.py` (pure, unit-tested) with EN/DE phrase tables ("press enter", "select all", "copy that"/"paste", "undo that"/"redo that", "delete last word"; German equivalents) matched exactly after normalization. A dedicated hold-to-command hotkey (`command_hotkey`, default `Ctrl+Shift+F10`) starts a command-mode recording (`recording["command"]`/`combo`, threaded through stop/drain/finish/transcribe); `execute_voice_command()`/`_tap_keys()` (`main.py`) run the keystrokes in the focused window, and an unmatched phrase falls through to normal dictation so nothing is lost. Command mode reuses the last-dictated profile's engine/language. `platform_mac.py` routes `_tap_keys` through the Tk main thread like paste/scratch. `--doctor` includes the hotkey in its conflict check, the tray menu lists it. Config: `voice_commands` (default on). Closes the feasible core of the old "Voice command grammar" Big Bet.
...
- **Voice command grammar (LLM-driven instructions)** — natural-language commands ("make this an email reply", "delete the last paragraph") need a local LLM; only the fixed-phrase subset is implemented (2026-09-20, see above).
